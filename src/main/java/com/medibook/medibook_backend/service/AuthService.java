...
public class AuthService {
    ...
    public PatientProfileDTO getPatientProfile(Long userId) {
        ...
        dto.setIdProofPath(patient.getIdProofPath());

        // Map Medical Records
        if (patient.getMedicalRecords() != null) {
            dto.setMedicalRecords(patient.getMedicalRecords().stream().map(record -> {
                MedicalRecordDTO recordDTO = new MedicalRecordDTO();
                recordDTO.setId(record.getId());
                recordDTO.setPatientId(record.getPatient().getId());
                recordDTO.setRecordType(record.getRecordType());
                recordDTO.setDescription(record.getDescription());
                recordDTO.setFileName(record.getFileName());
                recordDTO.setFileType(record.getFileType());
                recordDTO.setFileSize(record.getFileSize());
                recordDTO.setUploadDate(record.getUploadDate());
                return recordDTO;
